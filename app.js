  ...
  if (dbUser === undefined) {
    ...
    if (isPasswordMatch === false) {
      ...
      if (request.body.newPassword.length < 5) {
        ...
        const updatedUserHashedPassword = await bcrypt.hash(
          request.body.newPassword,
          10
        );
        const updateUserPasswordQuery = `UPDATE user SET password="${updatedUserHashedPassword}" WHERE username ="${username}";`;
